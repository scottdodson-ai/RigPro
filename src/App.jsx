      ...
      <div style={{ padding:"14px", maxWidth:1160, margin:"0 auto" }}>
        {selC ? (() => {
                          ...
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:7 }}>
                            ...
                            <div style={{ textAlign:"right", flexShrink:0 }}>
                              ...
                              <div style={{ display:"flex", gap:4, marginTop:5, justifyContent:"flex-end", flexWrap:"wrap" }}>
                                ...
                                {q.fromReqId && (() => { const rfq = reqs.find(r=>r.id===q.fromReqId); return rfq ? <button style={{ ...mkBtn("ghost"), padding:"3px 8px", fontSize:11 }} onClick={()=>setShowJFM(rfq)}>Job Folder</button> : null; })()}
          ...
          <>
            ...
            {custView === "card" ? (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(250px,1fr))", gap:10, gridAutoRows:"1fr" }}>
                {customers.filter(c=>(!search||c.name.toLowerCase().includes(search.toLowerCase()))&&(custFilter==="all"||(custFilter==="prospects"?c.isProspect:!c.isProspect))).map(c => {
                  ...
                  const closed   = c.quotes.filter(q=>["Won","Lost"].includes(q.status));
                  const winPct   = closed.length ? Math.round(won.length/closed.length*100) : null;
                  const openRfqs = reqs.filter(r=>r.company===c.name&&r.status!=="Dead"&&r.status!=="Quoted");
                  return (
                    <Card key={c.name} style={{ cursor:"pointer", marginBottom:0, display:"flex", flexDirection:"column" }} onClick={()=>setSelC(c.name)}>
                      {/* Name + badge */}
                      <div style={{ display:"flex", alignItems:"flex-start", gap:6, marginBottom:2, minHeight:44 }}>
                        <div style={{ fontWeight:700, fontSize:14, flex:1 }}>{c.name}</div>
                        {c.isProspect
                          ? <span style={{ background:"#fffbeb", color:"#b45309", border:"1px solid #fde68a", borderRadius:4, padding:"1px 7px", fontSize:10, fontWeight:700, flexShrink:0 }}>PROSPECT</span>
                          : <span style={{ background:C.grnB, color:C.grn, border:`1px solid ${C.grnBdr}`, borderRadius:4, padding:"1px 7px", fontSize:10, fontWeight:700, flexShrink:0 }}>CUSTOMER</span>
                        }
                      </div>
                      {/* Revenue */}
                      <div style={{ fontSize:18, fontWeight:700, color:C.acc, marginBottom:8 }}>{fmt(tot)}</div>
                      {/* Open RFQ alert — always same height, visible or invisible */}
                      <div style={{ height:29, marginBottom:8 }}>
                        {openRfqs.length > 0 && (
                          <div style={{ display:"flex", alignItems:"center", gap:5, background:C.bluB, border:`1px solid ${C.bluBdr}`, borderRadius:5, padding:"4px 9px", height:"100%", boxSizing:"border-box" }}>
                            <div style={{ width:7, height:7, borderRadius:"50%", background:C.blue, flexShrink:0 }}/>
                            <span style={{ fontSize:11, color:C.blue, fontWeight:700 }}>{openRfqs.length} open RFQ{openRfqs.length>1?"s":""}</span>
                          </div>
                        )}
                      </div>
                      {/* Stats circles — fixed size, equal spacing */}
                      <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                        {[
                          { label:"RFQs",   value:reqs.filter(r=>r.company===c.name).length, color:C.blue },
                          { label:"Quotes", value:c.quotes.length,                           color:C.acc  },
                          { label:"Win %",  value:winPct!==null?winPct+"%":"—",              color:winPct!==null&&winPct>=50?C.grn:winPct!==null?C.red:C.txtS },
                        ].map(x=>(
                          <div key={x.label} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:C.bg, border:`1px solid ${C.bdr}`, borderRadius:"50%", width:64, height:64 }}>
                            <div style={{ fontSize:15, fontWeight:800, color:x.color, lineHeight:1 }}>{x.value}</div>
                            <div style={{ fontSize:9, color:C.txtS, fontWeight:600, marginTop:2, textTransform:"uppercase", letterSpacing:.3 }}>{x.label}</div>
                          </div>
                        ))}
                      </div>
                      {/* Actions — pushed to bottom */}
                      <div style={{ display:"flex", gap:6, borderTop:`1px solid ${C.bdr}`, paddingTop:9, marginTop:"auto" }}>
              ...
              <Card style={{ padding:0 }}>
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse" }}>
                    <thead style={{ background:"#f9fafb" }}>
                      <tr>
                        {["Customer Name","Total Volume","Won Jobs","Open RFQs","Stats","Actions"].map(h => (
                    ...
                    <tbody>
                      {customers.filter(c=>(!search||c.name.toLowerCase().includes(search.toLowerCase()))&&(custFilter==="all"||(custFilter==="prospects"?c.isProspect:!c.isProspect))).map(c => {
                        ...
                        const winPct   = closed.length ? Math.round(won.length/closed.length*100) : null;
                        const openRfqs = reqs.filter(r=>r.company===c.name&&r.status!=="Dead"&&r.status!=="Quoted");
                        const totalRfqs = reqs.filter(r=>r.company===c.name).length;
                        return (
                          <tr key={c.name} style={{ cursor:"pointer" }} onClick={()=>setSelC(c.name)}>
                            <td style={{ ...tdS, padding:"12px 14px" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                                ...
                                {c.isProspect
                                  ? <span style={{ background:"#fffbeb", color:"#b45309", border:"1px solid #fde68a", borderRadius:4, padding:"1px 7px", fontSize:10, fontWeight:700 }}>PROSPECT</span>
                                  : <span style={{ background:C.grnB, color:C.grn, border:`1px solid ${C.grnBdr}`, borderRadius:4, padding:"1px 7px", fontSize:10, fontWeight:700 }}>CUSTOMER</span>
                                }
                              </div>
                            </td>
                            ...
                            <td style={{ ...tdS, padding:"12px 14px", whiteSpace:"nowrap" }}>
                              {/* Always-present open RFQ cell — invisible placeholder keeps column width */}
                              <div style={{ height:24 }}>
                                {openRfqs.length > 0 && (
                                  <span style={{ background:C.bluB, color:C.blue, border:`1px solid ${C.bluBdr}`, borderRadius:10, padding:"2px 8px", fontSize:10, fontWeight:700, whiteSpace:"nowrap" }}>
                                    {openRfqs.length} open RFQ{openRfqs.length>1?"s":""}
                                  </span>
                                )}
                              </div>
                            </td>
                            <td style={{ ...tdS, padding:"12px 14px" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"nowrap" }}>
                                {[
                                  { l:"RFQs",   v:totalRfqs,                            c:C.blue },
                                  { l:"Quotes", v:c.quotes.length,                      c:C.acc  },
                                  { l:"Win%",   v:winPct!==null?winPct+"%":"—",         c:winPct!==null&&winPct>=50?C.grn:winPct!==null?C.red:C.txtS },
                                ].map(x=>(
                                  <div key={x.l} style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:C.bg, border:`1px solid ${C.bdr}`, borderRadius:"50%", width:48, height:48, flexShrink:0 }}>
                                    <div style={{ fontSize:12, fontWeight:800, color:x.c, lineHeight:1 }}>{x.v}</div>
                                    <div style={{ fontSize:8, color:C.txtS, fontWeight:600, textTransform:"uppercase", letterSpacing:.3 }}>{x.l}</div>
                                  </div>
                                ))}
                              </div>
                            </td>
