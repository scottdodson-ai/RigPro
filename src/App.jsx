...
export function useTableSort(items, initialKey = null, initialDir = 1) {
  ...
  return { sortedItems, requestSort, sortKey, sortDir, setSortKey, setSortDir };
}
...
const Badge = ({ status }) => {
  let sName = status;
  if (!isNaN(status) && window.STATUS_LIST) {
    const st = window.STATUS_LIST.find(x => String(x.id) === String(status));
    if (st) sName = st.name;
  }
  const x = SS[sName] || SS.Draft;
  return <span style={{ background: x.bg, color: x.cl, border: `1px solid ${x.bd}`, borderRadius: 4, padding: "2px 8px", fontSize: 11, fontWeight: 600, whiteSpace: "nowrap" }}>{sName}</span>;
};
...
function Header({ view, setView, extra, crumb, role, token, setToken, setRole, profileUser, setProfileUser, customerCount, reqCount, quoteCount, jobCount, leadCount, siteCount }) {
  ...
  const TABS = token ? [
    ["dash", "Dashboard"], ["customers", customerCount !== undefined ? `Customers (${customerCount})` : "Customers"], ["sites", siteCount !== undefined ? `Sites (${siteCount})` : "Sites"], ["leads", leadCount !== undefined ? `Leads (${leadCount})` : "Leads"], ["quotes", quoteCount !== undefined ? `Quotes (${quoteCount})` : "Quotes"],
...
function buildReportData(reportId, jobs, reqs) {
  ...
  switch (reportId) {
    ...
    case "rev-by-customer": {
      ...
      return {
        cols: ["Customer", "Revenue"], clickHint: "Click a row to see individual jobs",
    ...
    case "rev-by-estimator": {
      ...
      const data = Object.values(m).sort((a, b) => b.revenue - a.revenue);
      return {
        cols: ["Estimator", "Revenue Won", "Quotes Won", "Quotes Lost", "Win Rate"], clickHint: "Click a row to see that estimator's jobs",
    ...
    case "rev-by-month": {
      ...
      return {
        cols: ["Month", "Revenue", "Jobs"], clickHint: "Click a month to see its jobs",
    ...
    case "pipeline-summary": {
      ...
      return {
        cols: ["Status", "Count", "Total Value", "Avg Deal", "Stale Count", "Stale Value"], clickHint: "Click a status to see its jobs",
        rows: data.map(r => {
    ...
    case "win-loss": {
      ...
      return {
        cols: ["Quote Type", "Won", "Lost", "Win Rate"], clickHint: "Click a type to see individual jobs",
    ...
    case "quote-aging": {
      ...
      return {
        cols: ["Quote #", "Customer", "Status", "Age", "Value"], clickHint: "Click a quote to open it",
    ...
    case "job-schedule": {
      ...
      return {
        cols: ["Job #", "Customer", "Description", "Start", "End", "Value"], clickHint: "Click a job to open its quote",
    ...
    case "cost-margin": {
      ...
      return {
        cols: ["Quote #", "Customer", "Revenue", "Est. Cost", "Gross Margin", "Margin %"], clickHint: "Click a row to see the full cost breakdown",
    ...
    case "estimator-activity": {
      ...
      return {
        cols: ["Estimator", "Total Quotes", "In Pipeline", "Won", "Lost", "Revenue Won", "Avg Margin %", "Avg Response (hrs)"], clickHint: "Click an estimator to see their jobs",
...
function QuotesPageView({ jobs, setJobs, setLeads, custData, setView, openEdit, selectedQuote, setSelectedQuote, role, profileUser, statusList }) {
  ...
  const toggleView = (v) => {
    if (v === "card") setSelectedQuote(null);
    setViewMode(v);
  };
  ...
  return (
    <div style={{ padding:"14px", maxWidth:1600, margin:"0 auto", width:"100%" }}>
      {/* CONTROLS PANEL */}
      <div className="sticky-search" style={{ position:"sticky", top:0, zIndex:10, background:C.bg, padding:"10px 0 20px 0", borderBottom:`2px solid ${C.accL}` }}>
        <div style={{ display:"flex", alignItems:"flex-end", gap:16, flexWrap:"wrap", maxWidth:1280, margin:"0 auto" }}>
          <div style={{ flex:1 }}>
             <div style={{fontSize:11, fontWeight:800, color:C.txtS, marginBottom:6, letterSpacing:0.8}}>SEARCH QUOTES</div>
             <div style={{position:"relative"}}>
               <input 
          ...
          <div style={{ display:"flex", flexDirection:"column" }}>
            ...
            <div style={{ display:"flex", background:C.bg, borderRadius:10, padding:4, border:`1px solid ${C.bdr}` }}>
              <button 
                ...
                style={{ background: viewMode === "card" ? "#fff" : "transparent", color: viewMode === "card" ? C.acc : C.txtM, border:"none", borderRadius:6, padding:"8px 16px", fontSize:13, fontWeight:700, cursor:"pointer", boxShadow: viewMode === "card" ? "0 2px 5px rgba(0,0,0,0.05)" : "none", transition:"all 0.2s" }}
              >
                <span style={{ marginRight:6 }}>🗂️</span> Grid
              </button>
              <button 
                onClick={() => toggleView("list")} 
                style={{ background: viewMode === "list" ? "#fff" : "transparent", color: viewMode === "list" ? C.acc : C.txtM, border:"none", borderRadius:6, padding:"8px 16px", fontSize:13, fontWeight:700, cursor:"pointer", boxShadow: viewMode === "list" ? "0 2px 5px rgba(0,0,0,0.05)" : "none", transition:"all 0.2s" }}
              >
      ...
      {/* DATA TABLE / GRID */}
      <div className={viewMode === "list" ? "app-split-layout" : ""} style={{ display: viewMode === "list" ? "grid" : "block", gridTemplateColumns: viewMode === "list" ? "420px 1fr" : "none", gap:30, marginTop: 15, alignItems:"flex-start" }}>
        
        {/* LEFT: LIST VIEW OR GRID */}
        {(!selectedQuote || viewMode === "list") && (
        <div className="leads-left-col" style={{ background: viewMode === "list" ? "#fff" : "transparent", border: viewMode === "list" ? `1px solid ${C.bdrL}` : "none", borderRadius:20, overflow:"hidden", display:"flex", flexDirection:"column", height: viewMode === "list" ? "calc(100vh - 160px)" : "auto" }}>
          <div style={{ overflow:"auto", flex:1, padding: viewMode === "list" ? 15 : 0 }}>
            {viewMode === "list" ? (
              <div style={{ display: "flex", flexDirection: "column" }}>
                {filtered.map(q => {
                    ...
                    >
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                        ...
                        <div style={{ textAlign: "right" }}>
                          ...
                          <Badge status={(statusList || []).find(s => String(s.id) === String(q.status))?.name || q.status || 'Draft'} />
                        </div>
                      </div>
            ...
            {filtered.length === 0 && <div style={{ padding: 40, textAlign: "center", color: C.txtM }}>No quotes found.</div>}
          </div>
        </div>
        )}
        ...
        {/* RIGHT: DETAILS VIEW */}
        {(selectedQuote || viewMode === "list") && (
        <div className="leads-right-col" style={{ background:"#fff", borderRadius:20, boxShadow:"0 5px 25px rgba(0,0,0,0.03)", border:`1px solid ${C.bdrL}`, height:"calc(100vh - 160px)", overflowY:"auto", display:"flex", flexDirection:"column" }}>
          {selectedQuote ? (
             <div style={{ flex:1, display:"flex", flexDirection:"column" }}>
               {(viewMode === "card" || viewMode === "list") && (
                 <div className={viewMode === "list" ? "mobile-only-return-btn" : ""} style={{ margin: "20px 32px 0 32px" }}>
                   <button style={{ padding: "8px 16px", borderRadius: 8, fontSize: 13, border: `1px solid ${C.bdr}`, color: C.txtM, background: "#fff", cursor:"pointer" }} onClick={() => setSelectedQuote(null)}>
                     ← Back to {viewMode === "list" ? "List" : "Grid"}
                   </button>
                 </div>
               )}
               <QuotePreviewPanel quote={selectedQuote} custData={custData} setJobs={setJobs} setLeads={setLeads} setSelectedQuote={setSelectedQuote} onEdit={() => openEdit(selectedQuote)} role={role} profileUser={profileUser} statusList={statusList} />
             </div>
          ) : (
      ...
      <style>{`
        @media (max-width: 1050px) {
          ...
          ${selectedQuote ? '.leads-left-col { display: none !important; }' : '.leads-right-col { display: none !important; }'}
        }
...
function QuotePreviewPanel({ quote, custData, onEdit, setJobs, setLeads, setSelectedQuote, role, profileUser, statusList = [] }) {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
            ...
            <Badge status={(statusList || []).find(s => String(s.id) === String(quote.status))?.name || quote.status || 'Draft'} />
            {(role || []).includes('admin') && (
              <select
                value={quote.status || ''}
                onChange={async (e) => {
                  ...
                  if (setLeads) {
                    if (String(newStatusId) === "1") {
                      // Moved TO Lead state, ensure it exists in leads list natively
                      const mappedQuote = { ...updatedQuote, customer_name: updatedQuote.customer_name || updatedQuote.client || '' };
                      setLeads(prev => {
                        if (prev.find(l => l.id === updatedQuote.id)) return prev.map(l => l.id === updatedQuote.id ? mappedQuote : l);
                        return [mappedQuote, ...prev];
                      });
                    } else {
                      // Not a lead, remove from leads strictly
                      setLeads(prev => prev.filter(l => l.id !== updatedQuote.id));
                    }
                  }
...
function MasterJobList({ jobs, reqs, jobFolders, openEdit, setShowJFM, onUpdateJobNum, onViewAttachments, jobListFilter, setJobListFilter, setView, setSelC, statusList }) {
...
function LeadRecordModal({ onClose, onSave, token, appUsers = [], custData = {}, CUSTOMERS = [], jobs = [], C, fmt, mkBtn, Sec, Lbl, Card, inp, sel, AutoInput }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Convert internal lead schema wrapper to Quote schema for unified persistence
      const payload = {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <Card>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 15 }}>
                ...
                <div>
                  <Lbl c="PRIMARY ADDRESS" />
                  <input style={{...inp, width: "100%", boxSizing: "border-box"}} value={lead.street} onChange={e => u("street", e.target.value)} placeholder="123 Main St" />
                </div>
                <div>
                  <Lbl c="CITY" />
                  <input style={{...inp, width: "100%", boxSizing: "border-box"}} value={lead.city} onChange={e => u("city", e.target.value)} placeholder="City" />
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                  <div style={{ flex: "1 1 100px" }}><Lbl c="STATE" /><input style={{...inp, width: "100%", boxSizing: "border-box"}} value={lead.state} onChange={e => u("state", e.target.value)} placeholder="ST" maxLength={2} /></div>
                  <div style={{ flex: "2 1 150px" }}><Lbl c="ZIP" /><input style={{...inp, width: "100%", boxSizing: "border-box"}} value={lead.zipcode} onChange={e => u("zipcode", e.target.value)} placeholder="ZIP" maxLength={10} /></div>
                </div>

                <div>
                  <Lbl c="ESTIMATOR" />
                  <select style={{ ...sel, width: "100%", boxSizing: "border-box" }} value={lead.estimator_id} onChange={e => u("estimator_id", e.target.value)}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                  <div style={{ flex: "1 1 140px" }}><Lbl c="CONTACT FIRST NAME" /><input style={{...inp, width: "100%", boxSizing: "border-box"}} value={lead.first_name} onChange={e => u("first_name", e.target.value)} /></div>
                ...
                <div>
                  ...
                  <textarea style={{ ...inp, width: "100%", boxSizing: "border-box", height: 80, resize: "vertical" }} value={lead.description} onChange={e => u("description", e.target.value)} placeholder="Detailed project description..." required />
                </div>
              </div>
            </Card>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 15 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            ...
            <div style={{ flex: "2 1 200px" }}>
              ...
              <input
                type="email"
                style={{ ...inp, width: "100%", boxSizing: "border-box", border: f.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.email) ? '1px solid #e74c3c' : inp.border }}
                value={f.email}
          ...
          <div>
            ...
            {(() => {
              ...
              return (
                <div style={{ width: "100%" }}>
                  {hasLocations && (
                    <select style={{ ...sel, width: "100%", boxSizing: "border-box", marginBottom: 8 }}
                  ...
                  {(!hasLocations || isCustom || !f.jobSite) && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      <div>
                        <input style={{ ...inp, width: "100%", boxSizing: "border-box" }} value={f.street || ""} placeholder="Street Address" onChange={e => u("street", e.target.value)} />
                      </div>
                      <div>
                        <input style={{ ...inp, width: "100%", boxSizing: "border-box" }} value={f.city || ""} placeholder="City" onChange={e => u("city", e.target.value)} />
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                        <div style={{ flex: "1 1 100px" }}>
                          <input style={{ ...inp, width: "100%", boxSizing: "border-box" }} value={f.state || ""} placeholder="State (OH)" onChange={e => u("state", e.target.value.toUpperCase().slice(0, 2))} maxLength={2} />
                        </div>
                        <div style={{ flex: "2 1 150px" }}>
                          <input style={{ ...inp, width: "100%", boxSizing: "border-box" }} value={f.zipcode || ""} placeholder="Zip" onChange={e => u("zipcode", e.target.value.replace(/[^0-9-]/g, "").slice(0, 10))} maxLength={10} />
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div>
            ...
            <textarea style={{ ...inp, width: "100%", boxSizing: "border-box", height: 80, resize: "vertical" }} spellCheck={true} autoCapitalize="sentences" autoCorrect="on" value={f.desc} onChange={e => u("desc", e.target.value)} onBlur={handleDescBlur} placeholder="Enter description... (Auto-corrects on blur)" />
            {!descOk && (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            ...
            <div style={{ flex: "1 1 120px" }}>
              ...
              <select style={{ ...sel, width: "100%", boxSizing: "border-box" }} value={f.salesAssoc || ""} onChange={e => u("salesAssoc", e.target.value)}>
                <option value="">Unassigned</option>
                {appUsers.filter(usr => usr.role === "estimator" || (usr.roles || []).includes("estimator")).map(u => (
      ...
      <div style={{ background: C.sur, borderRadius: 12, width: "100%", maxWidth: 920, boxShadow: "0 16px 48px rgba(0,0,0,.28)" }}>
        ...
        <div id="jfm-printable" style={{ display: "none" }}>
          ...
          <div class="section"><div class="section-title">Quote Progress</div>
            <div class="stages">
              {STAGES.map((s, i) => {
                const color = stageColors[i]; const done = i < stage; const active = i === stage;
...
function SalesAdjustmentModal({ quote, onSave, onClose }) {
  ...
  return (
    <div style={{
      position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 700,
      display: "flex", alignItems: "center", justifyContent: "center", padding: 16
    }}>
      <div style={{
        background: "#fff", borderRadius: 10, padding: 22, width: "100%", maxWidth: 480,
        ...
        {/* Current totals */}
        <div style={{
          background: "#f5f6f8", border: "1px solid #e2e5ea", borderRadius: 7,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <div style={{
                display: "flex", alignItems: "center", border: "1px solid #c8cdd5", borderRadius: 5,
                overflow: "hidden", background: "#fff", flex: 1
              }}>
                <span style={{
                  padding: "0 8px", color: "#8a93a2", fontSize: 14, borderRight: "1px solid #c8cdd5",
                  background: "#f9fafb", display: "flex", alignItems: "center", height: "100%"
                }}>$</span>
                <input type="number" min={0} step={0.01} value={amount} placeholder="0.00"
                  onChange={e => setAmount(e.target.value)}
                  style={{
                    border: "none", outline: "none", fontFamily: "inherit", fontSize: 14, padding: "7px 10px",
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 18, justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{
            background: "#fff", color: "#4a5060", border: "1px solid #e2e5ea",
            borderRadius: 6, padding: "7px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer"
          }}>Cancel</button>
          <button onClick={submit} style={{
            background: "#b86b0a", color: "#fff", border: "none",
...
function EquipmentPage({ equipment, setEquipment, eqCats, eqMap, eqOv, setEqOv, role }) {
  ...
  const fi = {
    background: C.sur, border: `1px solid ${C.bdrM}`, borderRadius: 5, color: C.txt,
    ...
    <div className="app-page-container" style={{ maxWidth: 1200 }}>
      ...
      {showAdd && (
        <Card style={{ marginBottom: 16, border: `1.5px solid ${C.accB}`, background: C.accL }}>
          ...
          {addError && (
            <div style={{
              background: C.redB, border: `1px solid ${C.redBdr}`, borderRadius: 5,
...
function LaborRatesPage({ customerRates, setCustomerRates, role, baseLabor, setBaseLabor }) {
  ...
  const fS = {
    background: "#fff", border: `1px solid ${C.bdrM}`, borderRadius: 5, color: C.txt,
...
function ProfileTemplateModal({ template, setTemplate, onClose }) {
  ...
  return (
    <div style={{
      position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", zIndex: 600,
      display: "flex", alignItems: "flex-start", justifyContent: "center",
      padding: "20px 12px", overflowY: "auto"
    }}>
      <div style={{
        background: "#fff", borderRadius: 10, padding: 22, width: "100%", maxWidth: 600,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
          {fields.map((f, i) => (
            <div key={f.key} style={{
              ...
              {/* Label edit */}
              <input style={{
                ...inp2, flex: 1, fontWeight: 600, fontSize: 13,
              ...
              {/* Type badge */}
              <span style={{
                fontSize: 10, color: "#8a93a2", background: "#f1f5f9", borderRadius: 3,
              ...
              <button onClick={() => moveUp(i)} disabled={i === 0}
                style={{
                  ...
                  color: i === 0 ? "#c8cdd5" : "#8a93a2", fontSize: 13, padding: "0 2px"
                }}>↑</button>
              <button onClick={() => moveDown(i)} disabled={i >= fields.length - 1}
                style={{
                  background: "none", border: "none", cursor: i >= fields.length - 1 ? "default" : "pointer",
                  color: i >= fields.length - 1 ? "#c8cdd5" : "#8a93a2", fontSize: 13, padding: "0 2px"
                }}>↓</button>
              {/* Remove */}
        ...
        <div style={{ background: "#f5f6f8", border: "1px solid #e2e5ea", borderRadius: 8, padding: 12, marginBottom: 14 }}>
          ...
          <button onClick={addField} style={{
            background: "#b86b0a", color: "#fff", border: "none", borderRadius: 5,
...
function CustomerModal({ custName, jobs, reqs = [], jobFolders = {}, custData, setCustData, profileTemplate, onOpenQuote, onOpenJobFolder, onClose }) {
  ...
  return (
    <div className="customer-profile-screen" style={{
      flex: 1, background: C.bg,
      display: "flex", alignItems: "flex-start", justifyContent: "center",
      padding: "40px 20px"
    }}>
      <div style={{
        background: "#fff", borderRadius: 12, width: "100%", maxWidth: 1200, minHeight: "85vh",
...
function CalendarPage({ jobs, setJobs, eqMap, onOpenQuote }) {
  ...
  function renderCell(d) {
    if (!d) return (
      <div style={{
        minHeight: 105, borderRight: `1px solid ${C.bdr}`,
    ...
    <div style={{ padding: "14px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      {showConflicts && (
        <div style={{
          ...
          <div style={{
            background: C.sur, borderRadius: 10, padding: 22, width: "100%", maxWidth: 580,
            ...
            {equipConflicts.map((cf, i) => (
              <div key={i} style={{
                background: C.redB, border: `1px solid ${C.redBdr}`,
                ...
                <div style={{ display: "flex", gap: 7, marginBottom: 10, flexWrap: "wrap" }}>
                  {cf.equipment.map(code => {
                    ...
                    return (
                      <span key={code} style={{
                        background: C.sur, border: `1px solid ${C.redBdr}`,
                ...
                {[cf.jobA, cf.jobB].map((job, i) => (
                  <div key={listKey("conflict-job", job, i)} style={{
      ...
      {detailJob && (
        <div style={{
          ...
          <div style={{
            background: C.sur, borderRadius: 10, padding: 20, width: "100%", maxWidth: 520,
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {/* Google Calendar */}
          <div style={{
            background: C.sur, border: `1px solid ${C.bdr}`, borderRadius: 8,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
        {/* View toggle */}
        <div style={{
          display: "flex", background: C.bg, border: `1px solid ${C.bdr}`,
        ...
        return (
          <div style={{ background: C.sur, border: `1px solid ${C.bdr}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
            ...
            {Object.entries(equipMap).filter(([, jbs]) => jbs.length > 1).map(([code, jbs]) => {
              ...
              return (
                <div key={code} style={{
                  background: C.redB, border: `1px solid ${C.redBdr}`, borderRadius: 7,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {jobs.map((job, i) => {
                ...
                return (
                  <div key={listKey("day-job", job, i)} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap", marginBottom: 3 }}>
                          ...
                          <span style={{
                            background: s.bg, color: s.cl, border: `1px solid ${s.bd}`,
                      ...
                      {(job.equipList || []).length > 0 && (
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 5 }}>
                          {(job.equipList || []).map(code => (
                            <span key={code} style={{
                              fontSize: 10, background: C.sur, border: `1px solid ${C.bdr}`,
  ...
  if (view === "jobs") return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.txt, fontFamily: "'Segoe UI','Helvetica Neue',Arial,sans-serif", fontSize: 14 }}>
      ...
      <MasterJobList
        ...
        jobListFilter={jobListFilter} setJobListFilter={setJobListFilter} setView={setView} setSelC={setSelC} statusList={statusList}
      />
      {attachModal && (
          ...
          <Card>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 15 }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "flex-end", gap: 10, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input style={{...inp, width: "100%", boxSizing: "border-box"}} value={active.street || ""} onChange={e => u("street", e.target.value)} placeholder="123 Main St" disabled={active.locked} />
                  </div>
                ...
                <div>
                  <Lbl c="CITY" />
                  <input style={{...inp, width: "100%", boxSizing: "border-box"}} value={active.city || ""} onChange={e => u("city", e.target.value)} placeholder="City" disabled={active.locked} />
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 10 }}>
                  <div style={{ flex: "1 1 100px" }}><Lbl c="STATE" /><input style={{...inp, width: "100%", boxSizing: "border-box"}} value={active.state || ""} onChange={e => u("state", e.target.value)} placeholder="State" disabled={active.locked} maxLength={2} /></div>
                  <div style={{ flex: "2 1 150px" }}><Lbl c="ZIP" /><input style={{...inp, width: "100%", boxSizing: "border-box"}} value={active.zipcode || ""} onChange={e => u("zipcode", e.target.value)} placeholder="ZIP" disabled={active.locked} maxLength={10} /></div>
                </div>
              </div>
              ...
              {active.status === "Lead" ? (
                <>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    <div style={{ flex: "1 1 140px" }}><Lbl c="CONTACT FIRST NAME" /><input style={{...inp, width: "100%", boxSizing: "border-box"}} value={active.first_name || ""} onChange={e => u("first_name", e.target.value)} disabled={active.locked} /></div>
              ...
              <div>
                <Lbl c="JOB DESCRIPTION" />
                <textarea style={{ ...inp, width: "100%", boxSizing: "border-box", height: active.status === "Lead" ? 80 : 56, resize: "vertical", marginBottom: 8 }} value={active.desc || ""} onChange={e => u("desc", e.target.value)} disabled={active.locked} placeholder={active.status === "Lead" ? "Detailed project description..." : ""} />
                {active.status !== "Lead" && (
                ...
                {active.status !== "Lead" && active.isHistorical && (
                  <div style={{ marginTop: 6 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, padding: 12, background: C.sur, border: `1px solid ${C.bdr}`, borderRadius: 6, alignItems: "end", marginBottom: 10 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 10, padding: 12, background: C.sur, border: `1px solid ${C.bdr}`, borderRadius: 6, alignItems: "end" }}>
                      ...
                      {active.status === "Won" && (
                        <>
                          <div style={{ flex: "1 1 140px" }}><Lbl c="ACTUAL START DATE" /><input type="date" style={{ ...inp, width: "100%", boxSizing: "border-box" }} value={active.startDate || ""} onChange={e => u("startDate", e.target.value)} disabled={active.locked} /></div>
                          <div style={{ flex: "1 1 140px" }}><Lbl c="ACTUAL END DATE" /><input type="date" style={{ ...inp, width: "100%", boxSizing: "border-box" }} value={active.compDate || ""} onChange={e => u("compDate", e.target.value)} disabled={active.locked} /></div>
                        </>
                      )}
                    </div>
                  </div>
